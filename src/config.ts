...
// Feature: Resource Provider API - Game Pass subscription cosigning
export const ENABLE_SUBSCRIPTION_TRANSACTIONS = isENVTrue(
	process.env.ENABLE_SUBSCRIPTION_TRANSACTIONS ?? 'false'
);
export const PROVIDER_SUBSCRIPTION_ENTITLEMENT_CONTRACT =
	process.env.PROVIDER_SUBSCRIPTION_ENTITLEMENT_CONTRACT;
export const PROVIDER_SUBSCRIPTION_ENTITLEMENT_SCOPE =
	process.env.PROVIDER_SUBSCRIPTION_ENTITLEMENT_SCOPE ?? PROVIDER_SUBSCRIPTION_ENTITLEMENT_CONTRACT;
export const PROVIDER_SUBSCRIPTION_ENTITLEMENT_TABLE =
	process.env.PROVIDER_SUBSCRIPTION_ENTITLEMENT_TABLE ?? 'plans';
export const PROVIDER_SUBSCRIPTION_ALLOWED_CONTRACTS = (
	process.env.PROVIDER_SUBSCRIPTION_ALLOWED_CONTRACTS ??
	...
	.filter(Boolean);
export const PROVIDER_SUBSCRIPTION_LIMIT_MS = process.env.PROVIDER_SUBSCRIPTION_LIMIT_MS
	? Number(process.env.PROVIDER_SUBSCRIPTION_LIMIT_MS)
	: 30000;
export const PROVIDER_SUBSCRIPTION_LIMIT_KB = process.env.PROVIDER_SUBSCRIPTION_LIMIT_KB
	? Number(process.env.PROVIDER_SUBSCRIPTION_LIMIT_KB)
	: 30000;
if (ENABLE_SUBSCRIPTION_TRANSACTIONS) {
	if (!ENABLE_RESOURCE_PROVIDER) {
		throw new Error(
			'If ENABLE_SUBSCRIPTION_TRANSACTIONS is set to true, ENABLE_RESOURCE_PROVIDER must also be set to true.'
		);
	}
	if (!ANTELOPE_NODEOS_API || !PROVIDER_SUBSCRIPTION_ENTITLEMENT_CONTRACT) {
		throw new Error(
			'Subscription transactions require ANTELOPE_NODEOS_API and PROVIDER_SUBSCRIPTION_ENTITLEMENT_CONTRACT.'
		);
	}
	if (!PROVIDER_SUBSCRIPTION_ALLOWED_CONTRACTS.length) {
		throw new Error(
			'Subscription transactions require at least one PROVIDER_SUBSCRIPTION_ALLOWED_CONTRACTS entry.'
		);
	}
	if (
		!Number.isFinite(PROVIDER_SUBSCRIPTION_LIMIT_MS) ||
		PROVIDER_SUBSCRIPTION_LIMIT_MS <= 0 ||
		!Number.isFinite(PROVIDER_SUBSCRIPTION_LIMIT_KB) ||
		PROVIDER_SUBSCRIPTION_LIMIT_KB <= 0
	) {
		throw new Error('Subscription CPU and NET limits must be positive numbers.');
	}
}
...
if (
	...
	throw new Error(
		'ENABLE_RESOURCE_PROVIDER requires free, subscription, paid, or light-account transactions to be enabled.'
	);
}
